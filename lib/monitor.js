        ...
        self.redis.keys(redisUtils.toKey('*', 'concurrency'), function (err, results) {
            ...
            async.forEachOf(jobs, function (value, jobType, callback) {
                self.redis.keys(redisUtils.toKey(jobType, 'jobs:*'), function (err, ids) {
                    ...
                    async.forEach(['active', 'pending', 'completed', 'failed', 'crashed', 'result'], function (queue, innerCB) {
                        if (queue === 'result') {
                            self.redis.hkeys(redisUtils.toKey(jobType, queue), function (err, result) {
                                _.difference(result, jobIds).forEach(function (resultId) {
