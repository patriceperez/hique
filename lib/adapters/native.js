...
module.exports = function (config) {
    ...
    self.finishJob = function (type, id, status, cb) {
        if (_.isObject(self.queues[type]) &&
            ...
            if (!(self.queues[type].removeOnSuccess)) {
                ...
                self.queues[type].jobs[id].endTime = Date.now();
    ...
    self.getStats = function (cb) {
        ...
        _.forOwn(self.queues, function (value) {
            stats.push({
                ...
                avg_duration: self.avgArr(_.filter(self.queues[value.type].jobs, function (job) {
    ...
    self.avgArr = function (jobs) {
        var sum = 0;
        var count = 0;
        for (var i = 0, ii = jobs.length; i < ii; ++i) {
            var job = jobs[i];

            if (typeof job.startTime !== 'undefined' && typeof job.endTime !== 'undefined') {
