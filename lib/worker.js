...
module.exports = function (config) {
    ...
    self.executeJob = function (job, jobLogic) {
        ...
        console.log(job.type, self.queues[job.type].jobs.length);
    ...
    self.createDoneMethod = function (job) {
        return function (err, result) {
            ...
            _.remove(self.queues[job.type].jobs, function (activeJob) {
    ...
    self.getFreeSlots = function () {
        ...
        _(self.queues).forEach(function (queue, type) {
            if (queue.jobs.length < queue.concurrency && typeof queue.jobLogic === 'function') {
